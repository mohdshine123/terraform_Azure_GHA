...
data "azurerm_virtual_network" "existing_vnet" {
  count               = azurerm_resource_group.my-terraform-rg[0].count
...
data "azurerm_subnet" "existing_subnet" {
  count                = azurerm_virtual_network.existing_vnet[0].count
  name                 = "my-terraform-subnet"
  resource_group_name  = azurerm_resource_group.my-terraform-rg[0].name
  virtual_network_name = azurerm_virtual_network.existing_vnet[0].name
}
